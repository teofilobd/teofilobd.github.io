...
Note that every option will appear only in the settings where they are needed. For example, the normal be will declared and used only when `USE_DIFFUSE`, `USE_SPECULAR_PHONG` or `USE_SPECULAR_BLINNPHONG` are set.
...
3. Loading several shader variants during the game can affect its performance (Create ShaderVariantCollections and preload them [LINK]).
